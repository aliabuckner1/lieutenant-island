...
function build(r,now){
  ...
  return {now:now,series:series,closures:closures,near:near,highs:highs,days:days,order:order,live:live,gaugeAt:gaugeAt,
...
function freshHTML(M,note){
  ...
  if(M.live==null)bits.push('<span class="warn">Boston gauge unavailable</span>');
...
window.Tides={load:load,run:run,ROAD:ROAD,NEAR:NEAR,MIN:MIN,HOUR:HOUR,DAY:DAY,STATES:STATES,stateFor:stateFor,
  nowW:nowW,dayStart:dayStart,minOfDay:minOfDay,fmtT:fmtT,fmtD:fmtD,fmtDur:fmtDur,longDur:longDur,
